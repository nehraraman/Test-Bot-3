...
def init_db():
    ...
    try:
        c = conn.cursor()
        # Create tables
        c.execute(ph("""CREATE TABLE IF NOT EXISTS users (
...
def update_setting(key, value):
    ...
    try:
        c = conn.cursor()
        if USE_SQLITE:
            c.execute(ph('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', (key, value))
        else:
...
async def tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        c = conn.cursor()
        c.execute(ph('SELECT task_id, title, description, link, reward FROM tasks')
        tasks = c.fetchall()
        if not tasks:
            keyboard = [[InlineKeyboardButton("🔙 Back", callback_data='back')]]
            reply_markup = InlineKeyboardMarkup(keyboard)
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        c = conn.cursor()
        c.execute(ph('SELECT username, coins FROM users ORDER BY coins DESC LIMIT 10')
